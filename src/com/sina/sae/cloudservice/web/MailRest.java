...
public class MailRest extends HttpServlet {
	...
	if (null != from && null != to && null != smtpHost && null != smtpUsername && null != smtpPassword && null != subject && null != content) {// 必选参数
	    ...
	    if (null != chartset)
		mail.setChartset(chartset);
	    if (null != compress)
		mail.setCompress(compress);
	    if (null != tls)
		mail.setTls(Boolean.parseBoolean(tls));
	    mail.setFrom(from);
	    ...
	    ;
	    logger.debug("send Mail from:" + from + "|to:" + to + "|cc:" + cc + "|smtpHost:" + smtpHost + "|smtpUsername:" + smtpUsername + "|smtpPassword:" + smtpPassword + "|subject:" + subject + "|content:" + content + "|contentType:"
		    + contentType + "|chartset:" + chartset + "|tls:" + tls + "|compress:" + compress + "|data:" + data);
	    writer.write(data);
	} else {
