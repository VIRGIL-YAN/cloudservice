...
<html>
  <head>
	...
	String sql = "select * from statistic_launch limit ?,10";
    Connection con = null;
	ResultSet rs = null;
	...
	try {
		...
		while(rs.next()){
			...
			row.put("osversion", rs.getString("osversion"));
			row.put("launchtimes", rs.getString("launchtimes"));
			row.put("inserttime", rs.getTimestamp("inserttime")+"");
			row.put("updatetime", rs.getTimestamp("updatetime")+"");
			row.put("networktype", rs.getInt("networktype")+"");
			list.add(row);
		}
	}catch(Exception e){
		e.printStackTrace();
     ...
     <div class="container-fluid">
      <div class="row-fluid">
          ...
          <div class="well sidebar-nav">
            <ul class="nav nav-list">
              ...
              <li class="nav-header">统计信息</li>
               <li  class="active"><a href="statisticuser.jsp">用户统计</a></li>
        ...
        <div class="span9" >
            <h4>用户信息统计</h4>
            <hr> 
            <form class="form-search" method="get" id="object" action="statisticuser.jsp">
		 	 <table class="table table-bordered"  >
				 <thead>
				  ...
				  %>
				  	<tr>
				  		<td><%=row.get("index") %></td>
				  		<td><%=row.get("deviceid") %></td>
				  		<td><%=row.get("model") %></td>
				  		<td><%=row.get("osversion") %></td>
				  		<td><%=row.get("launchtimes") %></td>
				  		<td><%=row.get("inserttime") %></td>
				  		<td><%=row.get("updatetime") %></td>
				  		<td><%=row.get("networktype") %></td>
				  	</tr>
				  <%}%>
				  </tbody>
			</table>
